...
Dim Shared As _Unsigned Long cWhite, cBlack, setColor, setColor1, menuColor, amendColor
...
' 0 = sprite
' 1 = tile
Dim Shared Sprite(1, 127, 255) As Integer
...
Dim Shared Tilemap(10, 1279) As Integer

' allow 4 palettes, index1 is edittype, index2 is paletteselected
Dim Shared MyPalette(1, 1, 255) As Integer
Dim Shared As Integer paletteSelected

' 0 = Sprite
' 1 = Tile
' 2 = TileMap
Dim Shared As Integer editType
Dim Shared As Integer stInd

Dim Shared As String myFilename, myDrive
...
cWhite = _RGB(255, 255, 255)
cBlack = _RGB(0, 0, 0)
setColor = _RGB(255, 0, 255)
setColor1 = _RGB(182, 255, 83)
menuColor = _RGB(255, 73, 0)
amendColor = _RGB(0, 255, 255)

Const pixels = 8
Const notSet = 999
Const Sprite_Edit = 0
Const Tile_Edit = 1
Const Tilemap_Edit = 2
Const menu_Base = 0
Const menu_File = 1
Const menu_Edit = 2
Const menu_EditType = 3
...
Sub initialDraw
    ...
    Else
        _PrintString (10, 480 - 20), myDrive + myFilename + ".stt"
    End If
End Sub
...
Sub DrawMenu (activeMenu As Integer)
    ...
    iC = 1
    Do While menuData(iC, activeMenu, editType, 1) <> 0
        lX = menuData(iC, activeMenu, editType, 1)
        lY = menuData(iC, activeMenu, editType, 2)
        hX = menuData(iC, activeMenu, editType, 3)
        hY = menuData(iC, activeMenu, editType, 4)
        tX = lX + 5
        tY = lY + 5
        Line (lX, lY)-(hX, hY), c, BF
        _PrintString (tX, tY), menuText(activeMenu, iC, editType)
        iC = iC + 1
    Loop
...
Sub drawPaletteGrid ()
    ...
    For l2 = 1 To y
        For l1 = 1 To x
            ...
            Line (x1, y1)-(x2, y2), cWhite, B
            If MyPalette(editType, paletteSelected, z) <> notSet Then
                Line (x1 + 1, y1 + 1)-(x2 - 1, y2 - 1), ColorValue(MyPalette(editType, paletteSelected, z)), BF
            End If
            z = z + 1
            x1 = x1 + pixels
        Next l1
    ...
    Line (530, iy)-(530 + pixels, iy + pixels), cWhite, B
    If canvasColour <> 0 Then
        Line (531, iy + 1)-(531 + pixels - 2, iy + 1 + pixels - 2), ColorValue(MyPalette(editType, paletteSelected, canvasColour)), BF
        c = MyPalette(editType, paletteSelected, canvasColour)
...
Sub draw512ColourGrid ()
    ...
    Line (530, iy)-(530 + pixels, iy + pixels), cWhite, B
    Line (531, iy + 1)-(531 + pixels - 2, iy + 1 + pixels - 2), ColorValue(sel512Colour), BF
...
Sub drawTilemapGrid ()
    ...
    For l2 = 0 To 31
        For l1 = 0 To 39
            ...
            y2 = y1 + pix + 1
            Line (x1, y1)-(x2, y2), cWhite, B
            Call paint1Tile(Tilemap(tilemapPage, (l2 * 40) + l1))
...
Sub paint1Tile (tileindex As Integer)
    ...
    y = 0
    For z = 0 To 63
        c1 = Sprite(stInd, tileindex, z)
        c = ColorValue(MyPalette(stInd, paletteSelected, c1))
        PSet (x + x1, y + y1), c
        x = x + 1
        If x > 7 Then
            x = 0
...
Sub handleMenuClick (activemenu As Integer, menuSelected As String)
    Select Case activemenu
        ...
        Case menu_Edit
            Select Case menuSelected
                Case "Copy"
                    Call clipBoardCopy(editType)
                Case "Cut"
                    Call clipBoardCut(editType)
                Case "Paste"
                    Call clipBoardPaste(editType)
            End Select
            activemenu = menu_Base
            Call initialDraw
        ...
        Case "0P"
            ...
            Select Case editType
                ...
                Case Tilemap_Edit
                    ...
                    If tilemapPage > max_pages - 1 Then
                        tilemapPage = max_pages - 1
                    Else
                        Call drawTilemapGrid
                    End If
            End Select
...
Function chkMenuClick% (x As Integer, y As Integer)
    ...
    mT = ""
    iC = 1
    Do While menuData(iC, activeMenu, editType, 1) <> 0 And mT = ""
        lX = menuData(iC, activeMenu, editType, 1)
        lY = menuData(iC, activeMenu, editType, 2)
        hX = menuData(iC, activeMenu, editType, 3)
        hY = menuData(iC, activeMenu, editType, 4)

        If x >= lX And x <= hX And y >= lY And y <= hY Then mT = menuText(activeMenu, iC, editType)
        iC = iC + 1
    Loop
...
Function chkPaletteClick% (x, y)
    ...
    If x >= lx And x <= hx And y >= ly And y <= hy Then
        ...
        If box > 0 Then
            Select Case MyPalette(editType, paletteSelected, box)
                Case notSet
                    MyPalette(editType, paletteSelected, box) = sel512Colour
                    canvasColour = box
                Case Else
                    sel512Colour = MyPalette(editType, paletteSelected, box)
                    canvasColour = box
            End Select
            chkPaletteClick = 1
            Call drawPaletteGrid
...
Function chkPaletteRClick% (x, y)
    ...
    If x >= lx And x <= hx And y >= ly And y <= hy Then
        ...
        If MyPalette(editType, paletteSelected, box) = notSet Then
            MyPalette(editType, paletteSelected, box) = sel512Colour
            canvasColour = box
            chkPaletteRClick = 1
            Call drawPaletteGrid
...
Function chkTilemapClick% (x, y)
    ...
    If x >= lx And x <= hx And y >= ly And y <= hy Then
        ...
        Else
            ...
            Put (z1, z2), bg1(), PSet
        End If
        Tilemap(tilemapPage, (bY * 40) + bX) = canvasIndex
        projectAmended = "Y"
    Else
        chkTilemapClick = 0
    End If
...
Sub NewProject ()
    ...
    For l = 1 To 255
        MyPalette(0, 0, l) = notSet
        MyPalette(0, 1, l) = notSet
        MyPalette(1, 0, l) = notSet
        MyPalette(1, 1, l) = notSet
    Next l
...
Sub loadProjData (indata As String)
    ...
    For l1 = 0 To rows
        ...
        For l2 = 0 To cols * 4 Step 4
            ...
            Select Case itemType
                Case "S"
                    Sprite(0, typeIndex, itemIndex) = mybyte
                Case "P"
                    Select Case typeIndex
                        Case 0
                            MyPalette(0, 0, itemIndex) = mybyte
                        Case 1
                            MyPalette(0, 1, itemIndex) = mybyte
                        Case 2
                            MyPalette(1, 0, itemIndex) = mybyte
                        Case 3
                            MyPalette(1, 1, itemIndex) = mybyte
                    End Select
                Case "T"
                    Sprite(1, typeIndex, itemIndex) = mybyte
                Case "M"
                    Tilemap(typeIndex, itemIndex) = mybyte
            End Select
...
Sub SaveProject ()
    ...
    saveFilename = _SaveFileDialog$("Save Project", myFilename, "*.stt", "")
    ...
    Open myDrive + myFilename + ".stt" For Output As #1
    Print #1, "' " + myFilename + ".stt"
...
Sub writeProjData (itemType As String, typeIndex As Integer)
    ...
    For l1 = 0 To itemcount
        ...
        Select Case itemType
            Case "S"
                mybyte = lead0(Sprite(0, typeIndex, l1), 3)
            Case "T"
                mybyte = lead0(Sprite(1, typeIndex, l1), 3)
            Case "P"
                Select Case typeIndex
                    Case 0
                        mybyte = lead0(MyPalette(0, 0, l1), 3)
                    Case 1
                        mybyte = lead0(MyPalette(0, 1, l1), 3)
                    Case 2
                        mybyte = lead0(MyPalette(1, 0, l1), 3)
                    Case 3
                        mybyte = lead0(MyPalette(1, 1, l1), 3)
                End Select
            Case "M"
                mybyte = lead0(Tilemap(typeIndex, l1), 3)
        End Select
...
Sub ImportSprite ()
    ...
    For y = 0 To gridYSize - 1
        For x = 0 To gridXSize - 1
            c = pixelColour(x, y)
            If c = t Then
                canvasColour = MyPalette(editType, paletteSelected, 0)
                Call setSpriteColour(canvasIndex, x, y)
            Else
                p = SearchAddPalette(editType, paletteSelected, c)
                canvasColour = p
                Call setSpriteColour(canvasIndex, x, y)
            End If
        Next x
...
Sub GenerateFiles ()
    ...
    Open myDrive + "generate.def" For Input As #1
    If _FileExists(myDrive + myFilename + ".inc") Then Kill myDrive + myFilename + ".inc"
    If _FileExists(myDrive + myFilename + ".bin") Then Kill myDrive + myFilename + ".bin"
    If _FileExists(myDrive + myFilename + ".txt") Then Kill myDrive + myFilename + ".txt"

    Open myDrive + myFilename + ".inc" For Output As #2
    Open myDrive + myFilename + ".bin" For Binary As #3
    Open myDrive + myFilename + ".txt" For Output As #4
    Do
        Line Input #1, indata
        If Not EOF(1) Then
            If Left$(indata, 1) = "{" Then
                ...
                Select Case func
                    Case "TILEPATTERNS"
                        Call GeneratePatterns(Tile_Edit)
                    Case "TILEPALETTE"
                        Call generatePalette(Tile_Edit, 0)
                    Case "SPRITEPATTERNS"
                        Call GeneratePatterns(Sprite_Edit)
                    Case "SPRITEPALETTE"
                        Call generatePalette(Sprite_Edit, 0)
                    Case "PAGEDATA"
...
Sub GeneratePatterns (patternType As Integer)
    Dim As Integer pixelCount, pixelsPerRow, pixelsPerByte
    Dim As Integer c, l, x, z
    Dim As String myByte, outData, pixel1, pixel2, itemType

    If patternType = Sprite_Edit Then
        pixelCount = 255
        pixelsPerRow = 16
        pixelsPerByte = 1
        itemType = "Sprite"
    ...
    For l = 0 To getItemGenCount(patternType)
        ...
        'Call writeFiles("defs " + lead0((pixelCount + 1) / pixelsPerByte, 3) + ", 0", 0, "Y", "N", "N")
        outData = "DB "

        For x = 0 To pixelCount Step pixelsPerByte

            If patternType = Sprite_Edit Then
                ' each pixel is 8 bits, so 1 pixels per byte
                c = Sprite(0, l, z)
                pixel1 = decToBin(c, 8)
...
Sub generatePalette (patternType As Integer, palNo As Integer)
    ...
    For l = 0 To getPaletteItemCount(patternType, palNo)
        c = MyPalette(patternType, palNo, l)
        If c = notSet Then c = 0
...
Sub generateMap (gentype As Integer)
    ...
    outdata = "DB "
    For x = 0 To 1279
        outdata = outdata + decToHex(Tilemap(l, x)) + ","
        Call writeFiles("", Tilemap(l, x), "N", "N", "Y")
        z = z + 1
        If z Mod 40 = 0 Then
...
Function getItemGenCount% (patterntype As Integer)
    ...
    For l1 = 0 To l
        For x1 = 0 To x
            Select Case patterntype
                Case Sprite_Edit
                    If Sprite(0, l1, x1) > 0 Then z = l1
                Case Tile_Edit
                    If Sprite(1, l1, x1) > 0 Then z = l1
                Case Tilemap_Edit
                    If Tilemap(l1, x1) > 0 Then z = l1
            End Select
        Next x1
    Next l1
    getItemGenCount = z
...
Function getPaletteItemCount (p As Integer, p1 As Integer)
    ...
    For l = 255 To 0 Step -1
        If MyPalette(p, p1, l) <> notSet Then
            z = l
            Exit For
        End If
    Next l
...
Function getSpriteColour~& (s As Integer, x As Integer, y As Integer)
    ...
    c1 = ColorValue(MyPalette(stInd, paletteSelected, c))
    getSpriteColour = c1
End Function
...
Sub setSpriteColour (s As Integer, x As Integer, y As Integer)
    ...
    If MyPalette(editType, paletteSelected, canvasColour) <> notSet Then
        If Sprite(editType, s1, offset) = 0 Then
            Sprite(editType, s1, offset) = canvasColour
        Else
            Sprite(editType, s1, offset) = 0
        End If
    End If
End Sub
...
Function getOffset% (x As Integer, y As Integer)
    ...
    ' an individual sprite or tile is 16x16 or 8x8 pixels but we store as an array 0-255 of integers
...
Sub clipBoardCopy (edittype As Integer)
    ...
    Select Case edittype
        ...
        Case Tilemap_Edit
            For x = 0 To 1279
                tilemapClipboard(x) = Tilemap(tilemapPage, x)
            Next x
            clipBoardhasData = 1
            clipBoardSource = edittype
...
Sub clipBoardCut (edittype As Integer)
    Dim As Integer x, y
    ...
    Select Case edittype
        Case Sprite_Edit, Tile_Edit
            ...
            For x = 0 To gridXSize - 1
                For y = 0 To gridYSize - 1
                    Sprite(edittype, getSpriteIndex(canvasIndex, x, y), getOffset(x, y)) = 0
                Next y
            Next x
            projectAmended = "Y"
        ...
        Case Tilemap_Edit
            Call clipBoardCopy(edittype)
            For x = 0 To 1279
                Tilemap(tilemapPage, x) = 0
            Next x
            projectAmended = "Y"
    End Select
End Sub

Sub clipBoardPaste (edittype As Integer)
    Dim As Integer x, y

    ' if the source was not the same as destination
    ' then try to get a decent palette colour match
    If edittype <> clipBoardSource Then
        For x = 0 To gridXSize - 1
            For y = 0 To gridYSize - 1
    ...
    Select Case edittype
        Case Sprite_Edit, Tile_Edit
            If clipBoardhasData = 1 Then
                For x = 0 To gridXSize - 1
                    For y = 0 To gridYSize - 1
                        Sprite(edittype, getSpriteIndex(canvasIndex, x, y), getOffset(x, y)) = clipBoard(x, y)
                    Next y
                Next x
            End If
            projectAmended = "Y"
        ...
        Case Tilemap_Edit
            If clipBoardhasData = 1 Then
                For x = 0 To 1279
                    Tilemap(tilemapPage, x) = tilemapClipboard(x)
                Next x
            End If
            projectAmended = "Y"
...
Function blah% (x As Integer, y As Integer)
    ...
    c = clipBoard(x, y)
    c2 = MyPalette(clipBoardSource, paletteSelected, c)
    diff1 = 512
    For z = 0 To 255
        c1 = MyPalette(editType, paletteSelected, z)
        diff = Abs(c1 - c2)
        If diff < diff1 Then
            diff1 = diff
            nearest = z
...
Sub setSpriteColour1 (x As Integer, y As Integer, myColour As Integer)
    ...
    Sprite(editType, s1, offset) = myColour
...
Function SearchAddPalette% (edittype As Integer, paletteSelected As Integer, c As Integer)
    Dim As Integer l, f, mp

    ' search the palette to see if colour c is present, return palette index if it is found
    f = -1
    For l = 1 To 255
        mp = MyPalette(edittype, paletteSelected, l)
        If mp = c Then
            f = l
            Exit For
        End If
    ...
    If f = -1 Then
        For l = 1 To 255
            ...
            If mp = notSet Then
                f = l
                MyPalette(edittype, paletteSelected, l) = c
                Exit For
            End If
        Next l
    End If
